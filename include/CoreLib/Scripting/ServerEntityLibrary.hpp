...
namespace bw
{
	class BURGWAR_CORELIB_API ServerEntityLibrary : public SharedEntityLibrary
	{
		...
		private:
			...
			void SetPosition(lua_State* L, entt::handle entity, const Nz::Vector2f& position) override;
			void SetRotation(lua_State* L, entt::handle entity, const Nz::DegreeAnglef& rotation) override;
			void UpdatePlayerJumpHeight(lua_State* L, entt::handle entity, float jumpHeight, float jumpHeightBoost) override;
			void UpdatePlayerMovement(lua_State* L, entt::handle entity, float movementSpeed) override;
	};
}
