...
namespace bw
{
	namespace
	{
		...
		const Nz::ObjectRef<ResourceType>& GetResource(const std::string& resourcePath, std::shared_ptr<VirtualDirectory>& assetDirectory, tsl::hopscotch_map<std::string, Nz::ObjectRef<ResourceType>>& cache)
		{
	...
	{
		return GetResource(soundPath, m_assetDirectory, m_soundBuffers);
	}

	const Nz::TextureRef& AssetStore::GetTexture(const std::string& texturePath) const
	{
		return GetResource(texturePath, m_assetDirectory, m_textures);
	}
}
