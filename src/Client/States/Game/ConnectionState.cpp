...
namespace bw
{
	...
	m_timeBeforeGivingUp(Nz::Time::Zero())
	{
		...
		m_clientSessionConnectedSlot.Connect(m_clientSession->OnConnected, [this] (ClientSession*)
		{
			...
			SwitchToState(std::make_shared<ConnectedState>(GetStateDataPtr(), m_clientSession, std::move(authState)), Nz::Time::Seconds(0.5));
		});
	...
	{
		bwLog(GetStateData().appComponent->GetLogger(), LogLevel::Error, "connection to address #{0}/{1} failed", m_currentAddressIndex + 1, m_addresses.size());
	...
	void ConnectionState::ProcessNextAddress()
	{
		...
		{
			bwLog(GetStateData().appComponent->GetLogger(), LogLevel::Error, "no more addresses available, connection failed");
	...
	void ConnectionState::ProcessNextAddress(const ServerName& name)
	{
		...
		{
			bwLog(GetStateData().appComponent->GetLogger(), LogLevel::Debug, "resolving {0}:{1}...", m_resolvingData->serverName.hostname, name.port);
			...
			else
			{
				...
				bwLog(GetStateData().appComponent->GetLogger(), LogLevel::Debug, "resolution of {0}:{1} succeeded ({2} address(es) found)", m_resolvingData->serverName.hostname, name.port, addresses.size());
	...
	void ConnectionState::ProcessNextAddress(const Nz::IpAddress& address)
	{
		...
		bwLog(GetStateData().appComponent->GetLogger(), LogLevel::Debug, "connecting to {0}...", address.ToString());

		auto sessionBridge = networkManager.ConnectToServer(address, 0);
		if (sessionBridge)
		{
			m_clientSession->Connect(std::move(sessionBridge));
			m_timeBeforeGivingUp = Nz::Time::Seconds(10.f);
	...
	{
		bwLog(GetStateData().appComponent->GetLogger(), LogLevel::Debug, "connecting using local session...");
		m_clientSession->Connect(sessionManager->CreateSession());
		m_timeBeforeGivingUp = Nz::Time::Seconds(3.f); //< Should be instant
	}
	...
	bool ConnectionState::Update(Nz::StateMachine& fsm, Nz::Time elapsedTime)
	{
		...
		else if (m_timeBeforeGivingUp > Nz::Time::Second())
		{
			if ((m_timeBeforeGivingUp -= elapsedTime) < Nz::Time::Second())
				m_clientSession->Disconnect();
		}
